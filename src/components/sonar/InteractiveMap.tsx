...
export function InteractiveMap() {
  ...
  useEffect(() => {
    ...
    const key = (event: KeyboardEvent) => {
      ...
      if (["+", "=", "-"].includes(event.key)) { state.zoom = Math.max(.65, Math.min(1.35, state.zoom + (event.key === "-" ? -.15 : .15))); }
    ...
    const wheel = (event: WheelEvent) => {
      ...
      event.preventDefault(); stopAuto();
      state.zoom = Math.max(.65, Math.min(1.35, state.zoom - event.deltaY * .001)); wake();
    };
